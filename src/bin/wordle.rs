...
impl Feedback {
	fn read() -> Self {
		...
		let unknowns = feedbacks
			.iter()
			.filter(|char_feedback| matches!(char_feedback, CharacterFeedback::Unknown))
			.count();
		...
		match unknowns {
			0 => {
				let corrects = feedbacks
					.iter()
					.filter(|char_feedback| matches!(char_feedback, CharacterFeedback::Correct))
					.count();
			...
			CHARACTERS_PER_WORD => Self::Unknown,
	...
	fn from_guess(guess: &Word, chosen: &Word) -> Self {
		...
		if char_feedback
			.iter()
			.all(|&feedback| feedback == CharacterFeedback::Correct)
		{
			return Self::Correct;
		}
...
impl CharacterRule {
	...
	fn satisfies(&self, word: &Word) -> bool {
		match *self {
			Self::ContainsCharacterHere { chr, idx } => word.has_char_at(&chr, idx),
			Self::ContainsCharacterElsewhere { chr, idx } => {
				!word.has_char_at(&chr, idx) && word.has_char(&chr)
			}
			Self::NotContainsCharacter { chr } => !word.has_char(&chr),
		}
	}
}
...
impl RuleSet {
	fn from_guess_feedback(
		guess: &Word,
		feedback: &[CharacterFeedback; CHARACTERS_PER_WORD],
	) -> Self {
		let rules: Vec<_> = guess
			.iter()
			.copied()
			.enumerate()
			.zip(feedback.iter())
			.map(|((idx, chr), &state)| CharacterRule::new(chr, idx, state))
			.collect();

		Self {
			rules: rules.try_into().unwrap(),
		}
	}

	fn satisfies(&self, word: &Word) -> bool {
		self.rules.iter().all(|rule| rule.satisfies(word))
	}
}
...
impl WordSolver {
	...
	fn read() -> Self {
		...
		let dictionary: Vec<_> = read_input::<String>()
			.split_whitespace()
			.map(String::from)
			.collect();
	...
	fn generate_guess_by_bruteforce(&self) -> Word {
		...
		DictionaryTreeNode::from_words(self.dictionary.iter().collect_vec()).get_guess()
	}
...
impl Word {
	...
	fn from_str(s: &str) -> Self {
		let characters: [char; CHARACTERS_PER_WORD] = s.chars().collect_vec().try_into().unwrap();
		Self::new(characters)
	}
...
fn entropy(probabilities: &[f32]) -> f32 {
	let s: f32 = probabilities
		.iter()
		.copied()
		.map(|p| if p == 0f32 { p } else { p * p.log2() })
		.sum();
	-s
}
...
impl<'a> DictionaryTreeNode<'a> {
	...
	fn partition(
		words: Vec<&'a Word>,
		idx: usize,
		open_characters: &HashSet<char>,
	) -> DictionaryTreeNode<'a> {
				...
				.map(|(chr_idx, chr)| {
					if idx < 3 {
						eprintln!(
							...
							chr_idx + 1,
							String::from("=").repeat(idx),
							String::from(chr)
						);
					}

					let (exact, misplaced, missing) =
						PartitionBy::partition(&words, chr, idx).split();
	...
	fn from_words(words: Vec<&'a Word>) -> Self {
		let open_characters = words.iter().fold(HashSet::new(), |mut acc, word| {
...
fn main() {
	...
	let mut precaculated_guesses: VecDeque<_> =
		vec!["BONIER"].into_iter().map(Word::from_str).collect();
...
mod tests {
	...
	fn extract_initial_word() {
		...
		let permutations = words.iter().flat_map(Word::permutations).collect_vec();

		let chunk_size = (permutations.len() / N_THREADS) + 1;

		let handles = permutations
			.into_iter()
			.chunks(chunk_size)
			.into_iter()
			.map(|words_chunk| {
				...
				thread::spawn(move || {
					let root = words_chunk
						.into_iter()
						.map(|word| DictionaryTreeNode::by_word(words.iter().collect_vec(), &word))
	...
	fn test_parallel() {
		...
		eprintln!(
			"Total guesses: {:?}, average {}",
			n_tries,
			(*n_tries as f32) / (words.len() as f32)
		)
	}
